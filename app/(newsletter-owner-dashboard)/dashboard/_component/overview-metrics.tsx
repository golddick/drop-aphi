...
//   TopBlog, 
//   TopCampaign 
// } from "@/actions/analytics/overview-metrics-actions"
...
//               Growth Trends
//             </CardTitle>
//             <CardDescription>Subscriber, email, and blog growth over time</CardDescription>
//           </CardHeader>
...
//                       name="Subscribers"
//                     />
//                     <Area type="monotone" dataKey="emails" stackId="2" stroke="#10B981" fill="#86EFAC" name="Emails Sent" />
//                     <Area type="monotone" dataKey="blogViews" stackId="3" stroke="#F59E0B" fill="#FDE68A" name="Blog Views" />
//                   </AreaChart>
//                 </ResponsiveContainer>
//               </div>
//             )}
...
//                         <Progress value={campaign.clickRate * 10} className="w-12 h-1.5 mt-1" />
//                       </div>
//                     </div>
//                   </div>
//                 ))}
...
export function OverviewMetrics({ timeRange }: OverviewMetricsProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Blog Performance:', blogPerf)
  ...
  // Filter out months with all zero data to avoid empty charts
  const getFilteredGrowthData = () => {
    if (growthData.isEmpty || !growthData.data) return []
    return growthData.data.filter(month => 
      month.subscribers > 0 || month.emails > 0 || month.blogViews > 0
    )
  }

  const getFilteredPerformanceData = () => {
    if (performanceData.isEmpty || !performanceData.data) return []
    return performanceData.data.filter(month => 
      month.openRate > 0 || month.clickRate > 0 || month.blogEngagement > 0
    )
  }

  const filteredGrowthData = getFilteredGrowthData()
  ...
  // Custom YAxis formatter for large numbers
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Subscriber, email, and blog growth over time
              {filteredGrowthData.length > 0 && (
                <span className="text-xs text-gray-500 ml-2">
                  ({filteredGrowthData.length} months with data)
                </span>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            {growthData.isEmpty || filteredGrowthData.length === 0 ? (
              <EmptyChartState
                icon={TrendingUp}
                title="No Growth Data"
                message={growthData.message || "Start by adding subscribers, sending emails, or publishing blog posts to see growth trends."}
        ...
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Email and blog engagement rates
              {filteredPerformanceData.length > 0 && (
                <span className="text-xs text-gray-500 ml-2">
                  ({filteredPerformanceData.length} months with data)
                </span>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            {performanceData.isEmpty || filteredPerformanceData.length === 0 ? (
              <EmptyChartState
                icon={Eye}
                title="No Performance Data"
                message={performanceData.message || "Send some emails and publish blog posts to see engagement metrics."}
      ...
      {!blogPerformance.isEmpty && blogPerformance.data && blogPerformance.data.length > 0 && (
        <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center text-lg sm:text-xl">
            ...
            <CardDescription>Monthly blog views, read rates, and shares</CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="h-[300px] sm:h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart 
                  data={blogPerformance.data} 
                  margin={{ top: 10, right: 10, left: 10, bottom: 20 }}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
                  <XAxis 
