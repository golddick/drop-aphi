...
export function DraggableComponent({ 
  ...
  return (
    <Card
      ...
      {...listeners}
      {...attributes}
      className={`${sizeClasses.card} bg-sidebar-accent hover:bg-sidebar-accent/80 border-sidebar-border cursor-grab active:cursor-grabbing transition-colors ${
        isDragging ? "opacity-50 z-50" : ""
      }`}
    >
      <div className="flex items-center gap-3">
        <Icon className={`${sizeClasses.icon} text-sidebar-primary`} />
        <span className={`${sizeClasses.text} text-sidebar-foreground`}>{label}</span>
      </div>
    </Card>
  )
}