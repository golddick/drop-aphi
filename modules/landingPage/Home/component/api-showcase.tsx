...
function APIShowcaseItem({
  ...
  return (
    <div
      ref={ref}
      className={`grid lg:grid-cols-2 gap-8 md:gap-12 items-center mb-16 md:mb-20 ${isVisible ? "animate-slide-in-up" : "opacity-0"}`}
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className={reversed ? "lg:order-1 order-1" : "order-1"}>
        <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
          <div className="p-3 md:p-4 bg-gradient-to-br from-yellow-100 to-red-100 rounded-xl">
            {icon}
          </div>
        ...
        <Button className="bg-red-600 text-white hover:bg-black flex items-center gap-2 w-full sm:w-auto">
          Explore API <ArrowRight className="w-4 h-4" />
        </Button>
      </div>

      <div className={reversed ? "lg:order-2 order-2" : "order-2"}>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-6 overflow-hidden">
          <div className="text-xs md:text-sm text-gray-400 mb-3 md:mb-4 font-mono">
            {codeLabel}
          </div>
          <div className="overflow-x-auto">
            <pre className="text-xs md:text-sm text-gray-300 font-mono leading-relaxed min-w-min">
              <code>{codeSnippet}</code>
            </pre>
          </div>
        </div>
...
export default function APIShowcase() {
  return (
    <section id="api" className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 md:mb-20">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-black mb-4 px-4">
            <span className="text-black">Three Powerful APIs,</span>{" "}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-red-600 to-red-600">
              Endless Possibilities
            </span>
        ...
        <APIShowcaseItem
          icon={<Mail className="w-6 h-6 md:w-8 md:h-8 text-yellow-600" />}
          title="Newsletter & Email Builder API"
          description="Create stunning newsletters with our drag-and-drop builder or use our REST API to send programmatic emails. Complete subscriber management, analytics, and templates included."
          features={["Drag & Drop Builder", "Custom Templates", "Subscriber Management", "Analytics & Tracking"]}
          codeLabel="POST /api/v1/newsletters/send"
  ...
  "schedule": "2024-01-15T10:00:00Z"
        ...
        <APIShowcaseItem
          icon={<Shield className="w-6 h-6 md:w-8 md:h-8 text-red-600" />}
          title="OTP Verification API"
          description="Secure user authentication with one-time passwords. No SMTP configuration needed - we handle delivery, rate limiting, and security automatically."
          features={["Instant Delivery", "Rate Limiting", "Custom Templates", "Security Built-in"]}
          codeLabel="POST /api/v1/otp/send & verify"
...
// Verify OTP
{
  ...
  "code": "123456"
        ...
        <APIShowcaseItem
          icon={<Code className="w-6 h-6 md:w-8 md:h-8 text-yellow-600" />}
          title="Blog Content API"
          description="Headless CMS for blog content with customizable templates. Fetch and display blog posts on any website with our flexible API and template system."
          features={["Headless CMS", "Custom Templates", "SEO Optimized", "Cross-Platform"]}
          codeLabel="GET /api/v1/blog/posts"
